  ...
  useEffect(() => {
    fetchPopulationByAges(countryCode).then(
      ({ child, teenage, adult, elderly }) => {
        setPopulationByAges([
          !child[1] ? NaN : parseFloat(child[1][5].value?.toFixed(3)),
          !teenage[1] ? NaN : parseFloat(teenage[1][5].value?.toFixed(3)),
          !adult[1] ? NaN : parseFloat(adult[1][5].value?.toFixed(3)),
          !elderly[1] ? NaN : parseFloat(elderly[1][5].value?.toFixed(3)),
        ]);
      }
    );
  }, [countryCode]);
